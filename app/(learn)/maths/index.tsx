...
const PROGRAM_ID = process.env.EXPO_PUBLIC_MATHS_PROGRAM_ID ?? 'maths-pilot';

export default function MathsHomeScreen() {
  const router = useRouter();
  const [lessons, setLessons] = useState<Lesson[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getLessons(PROGRAM_ID, 'maths').then(setLessons).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>🔢 Maths</Text>
      <Text style={styles.subtitle}>Numbers, Shapes & Problem Solving</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            onPress={() => router.push(`/(learn)/maths/${item.id}` as never)}
          >
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 26, fontWeight: '800', color: '#7C3AED', padding: 20, paddingTop: 60, paddingBottom: 4 },
  ...
  lessonNumber: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#EDE9FE', justifyContent: 'center', alignItems: 'center' },
  lessonNumberText: { fontWeight: '700', color: '#7C3AED', fontSize: 14 },
  info: { flex: 1 },
