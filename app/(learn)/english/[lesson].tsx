...
export default function EnglishLessonScreen() {
  ...
  async function handleComplete() {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          { text: 'Skip for now', onPress: () => router.back() },
          { text: 'Start Quiz', onPress: () => router.push(`/(learn)/quiz/${session.id}` as never) },
        ],
      );
    } finally {
      setCompleting(false);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.meta}>{lesson.durationMinutes} min · English</Text>
      ...
      >
        <Text style={styles.completeButtonText}>
          {completing ? 'Saving...' : 'Mark Complete & Take Quiz'}
        </Text>
      </TouchableOpacity>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 24, fontWeight: '800', color: '#2563EB' },
  meta: { fontSize: 13, color: '#888' },
  videoContainer: { width: '100%', aspectRatio: 16 / 9, borderRadius: 12, overflow: 'hidden', backgroundColor: '#000' },
  video: { flex: 1 },
  completeButton: { backgroundColor: '#2563EB', borderRadius: 12, paddingVertical: 16, alignItems: 'center', marginTop: 8 },
