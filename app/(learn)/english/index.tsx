...
// Pilot: English as a Second Language program
const PROGRAM_ID = process.env.EXPO_PUBLIC_ESL_PROGRAM_ID ?? 'esl-pilot';
...
export default function EnglishHomeScreen() {
  ...
  useEffect(() => {
    getLessons(PROGRAM_ID, 'english').then(setLessons).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>🗣️ English</Text>
      <Text style={styles.subtitle}>English as a Second Language — Pilot Program</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            onPress={() => router.push(`/(learn)/english/${item.id}` as never)}
          >
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 26, fontWeight: '800', color: '#2563EB', padding: 20, paddingTop: 60, paddingBottom: 4 },
  ...
  lessonNumber: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#DBEAFE', justifyContent: 'center', alignItems: 'center' },
  lessonNumberText: { fontWeight: '700', color: '#2563EB', fontSize: 14 },
  info: { flex: 1 },
